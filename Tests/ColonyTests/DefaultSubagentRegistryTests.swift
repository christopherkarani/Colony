...
private enum SyntheticGraphProviderError: Error {
    case compileFailed
}
...
func defaultSubagentRegistry_surfacesGraphProviderFailure() async throws {
    let registry = ColonyDefaultSubagentRegistry(
        profile: .onDevice4k,
        modelName: "test-subagent-model",
        model: AnyHiveModelClient(GeneralPurposeDelegatingModel()),
        ...
        filesystem: nil,
        graphProvider: {
            throw SyntheticGraphProviderError.compileFailed
        }
    )
    ...
    do {
        ...
        #expect(errorDescription.contains("graphCompilationFailed") == true)
    }
}
