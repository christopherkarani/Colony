...
package protocol ColonySwarmToolBridging: ColonyToolRegistry, Sendable {
    ...
    var requiredCapabilities: ColonyRuntimeCapabilities { get }

    /// Risk-level overrides for Colony's safety policy engine.
    var riskLevelOverrides: [ColonyToolName: ColonyToolRiskLevel] { get }

    /// Policy metadata derived from Swarm tool execution semantics.
    var toolPolicyMetadataByName: [ColonyToolName: ColonyToolPolicyMetadata] { get }

    /// Returns Hive tool definitions filtered by the active capabilities.
    func listHiveTools(filteredBy activeCapabilities: ColonyRuntimeCapabilities) -> [HiveToolDefinition]
